...
namespace amp
{
	...
	public:
		...
		{
			AMP_ASSERT((data && size >= 0) || (size == 0 && !data));
			m_data = data;
			m_size = size;
		}
	...
	public:
		...
		constexpr span min_subspan(size_type offset, size_type count) const noexcept
		{
	...
	public:
		...
		const reference operator[](size_type idx) const noexcept
		{
